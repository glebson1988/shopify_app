...
class ProductEdit extends Component {
    ...
    addBlock(image_url) {
        ...
        this.setState({
            blocks: [...this.state.blocks,
                {
                    block_id: Math.round(Math.random() * 1000), block_text: 'new block',
                    block_type: this.state.currentBlockType, image_url
                }]
        })
        this.closeModal()
    }
    ...
    saveLookBook() {
        ...
        this.setState({stored_product}, () => {
            ...
            fetch(Routes.api_v1_stored_product_path(this.props.product.id,
                {format: 'json'}), {
                method: 'PUT',
                headers: {
        ...
        return (
            ...
            >
                <Page
                    ...
                    <Card title={static_text.choose_block} sectioned>
                        <Stack distribution='fill'>
                            <Button size='medium' onClick={() =>
                                this.openModal('Block3')}>{static_text.block3_title}</Button>
                            <Button size='medium' onClick={() =>
                                this.openModal('Block1')}>{static_text.block1_title}</Button>
                            <Button size='medium' onClick={() =>
                                this.openModal('Block2')}>{static_text.block2_title}</Button>
                            <Button destructive size='medium' onClick={() =>
                                this.deleteLastBlock()}>{static_text.delete_block_title}</Button>
                        </Stack>
                    </Card>
                    <Modal
                        ...
                        <div id='allBlocks'>
                            {this.state.blocks.map((block, i) => {
                                switch (block.block_type) {
                                    case "Block1":
                                        return <Block1 key={i} image_url={block.image_url} text={block.block_text}/>
                                    case  "Block2":
                                        return <Block2 key={i} image_url={block.image_url} text={block.block_text}/>
                                    case "Block3":
                                        return <Block3 key={i} image_url={block.image_url} text={block.block_text}/>
                                }
                            })}
                        </div>
                    </Card>
